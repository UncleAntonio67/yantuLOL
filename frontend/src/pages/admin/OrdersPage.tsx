...
import type { DeliverResponse, Order, OrderPage, Product, SendEmailResponse } from "../../lib/types";
...
function toIsoForQuery(value: string) {
  const v = String(value || "").trim();
  if (!v) return "";
  // datetime-local is interpreted as local time by JS Date.
  const d = new Date(v);
...
export default function OrdersPage() {
  ...
  async function loadProducts() {
    const ps = await apiJson<Product[]>("/api/admin/products");
    setProducts(ps);
  ...
  useEffect(() => {
    loadProducts().catch((ex: any) => setErr(ex?.message || "加载失败"));
  ...
  useEffect(() => {
    loadOrders();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, buyerIdQuery, statusFilter, createdFromQuery, createdToQuery]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="新建发货" subtitle="手动创建订单并生成链接与密码" className="lg:col-span-1">
          <form
            className="space-y-4"
            onSubmit={async (e) => {
              e.preventDefault();
              ...
              <div className="rounded-2xl border border-brand-200 bg-brand-50 p-4 text-sm space-y-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-xs text-gray-600">
                      ...
                      <span className="font-semibold text-gray-900">
                        {deliverRes.delivery_method === "text"
                          ? "图文私信文案"
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <div>
                    ...
                    <div className="font-mono font-bold text-brand-800">{deliverRes.password}</div>
                  </div>
                </div>
                  ...
                  <div className="rounded-2xl border border-brand-200 bg-white/70 p-3">
                    ...
                    <div className="mt-2 flex flex-col sm:flex-row gap-3 sm:items-start">
                      ...
                      <div className="text-xs text-gray-600 leading-6">
                        ...
                        <div className="mt-2">
                          <Button tone="ghost" size="sm" type="button" onClick={() => window.open(deliverRes.viewer_url, "_blank")}>
                            打开阅读页预览
                          </Button>
                        </div>
                      </div>
                ...
                {deliverRes.delivery_method === "email" ? (
                  <div className="rounded-2xl border border-brand-200 bg-white/70 p-3 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button tone="ghost" size="sm" type="button" onClick={() => copy(ensureDisclaimer(emailBody, deliverRes.legal_disclaimer))}>
                        复制邮件正文
                      </Button>
                      <Button
                        size="sm"
                  ...
                  <div className="rounded-2xl border border-brand-200 bg-white/70 p-3 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button tone="ghost" size="sm" type="button" onClick={() => copy(ensureDisclaimer(notifyText, deliverRes.legal_disclaimer))}>
                        复制文案
                      </Button>
                      <Button tone="ghost" size="sm" type="button" onClick={() => window.open(deliverRes.viewer_url, "_blank")}>
                        打开阅读页
        ...
        <Card title="发货记录 & 售后" subtitle="支持查询、确认收货、退款吊销、重置密码" className="lg:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 w-full">
            ...
            <div>
              <Label>开始时间</Label>
              <Input type="datetime-local" value={createdFromInput} onChange={(e) => setCreatedFromInput(e.target.value)} />
            </div>
            <div>
              <Label>结束时间</Label>
              <Input type="datetime-local" value={createdToInput} onChange={(e) => setCreatedToInput(e.target.value)} />
            </div>
            <div>
              <Label>状态</Label>
              <Select
            ...
            <div className="flex items-end gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setCreatedFromQuery(toIsoForQuery(createdFromInput));
                  setCreatedToQuery(toIsoForQuery(createdToInput));
                  setPage(1);
                }}
                className="w-full"
              >
          ...
          {loading ? (
            ...
            <div className="mt-4 space-y-3">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {orders.map((o) => (
                      <tr key={o.id} className="hover:bg-white/60 align-top">
                        <td className="px-3 py-3 font-mono text-xs" title={o.id}>
                          {shortId(o.id)}
                        </td>
                        <td className="px-3 py-3 font-semibold text-brand-700 truncate" title={o.buyer_id}>
                          {o.buyer_id}
                        </td>
                        <td className="px-3 py-3">
                          <div className="font-semibold text-gray-900 truncate" title={o.product_name}>
                            {o.product_name}
                          </div>
                          <div className="text-xs text-gray-500 truncate">操作人: {o.operator_nickname}</div>
                        </td>
                        <td className="px-3 py-3">
                        ...
                        <td className="px-3 py-3">
                          {o.status !== "active" ? (
                            ...
                            <span className="text-green-700 font-semibold">已</span>
                          ) : (
                            <span className="text-amber-700 font-semibold">待</span>
                          )}
                        </td>
                        <td className="px-3 py-3 font-semibold text-gray-900">{o.is_confirmed && o.status === "active" ? `¥${o.unit_price}` : "-"}</td>
                        <td className="px-3 py-3 font-mono text-xs">{o.password_last4}</td>
                        <td className="px-3 py-3 text-xs text-gray-700">{fmtDate(o.created_at)}</td>
                        <td className="px-3 py-3 text-right">
                          <div className="relative inline-flex items-center justify-end gap-2" data-menu-root>
                            <Button tone="ghost" size="sm" type="button" onClick={() => window.open(`/view/${o.id}`, "_blank")}>
                              阅读
                            </Button>
                            <Button
                              tone="ghost"
                              size="sm"
                              type="button"
                              onClick={() => setOpenMenuFor((prev) => (prev === o.id ? null : o.id))}
                            >
                              操作
                            </Button>

                            {openMenuFor === o.id && (
                              <div className="absolute right-0 top-[calc(100%+8px)] w-44 rounded-2xl border border-gray-100 bg-white shadow-soft p-2 text-left z-20">
              ...
              <div className="md:hidden space-y-3">
                {orders.map((o) => (
                  <div key={o.id} className="rounded-2xl border border-gray-100 bg-white/80 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="font-mono text-xs text-gray-600 break-all">{o.id}</div>
                        <div className="mt-1 font-black text-gray-900 truncate">{o.product_name}</div>
                        <div className="mt-1 text-xs text-gray-600">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-700">
                      <div className="rounded-xl border border-gray-100 bg-white/70 px-3 py-2">
                        ...
                        <div className="mt-1 font-semibold text-gray-900">{o.operator_nickname}</div>
                      </div>
                      <div className="rounded-xl border border-gray-100 bg-white/70 px-3 py-2">
                        <div className="text-[11px] text-gray-500">营收</div>
                        <div className="mt-1 font-semibold text-gray-900">{o.is_confirmed && o.status === "active" ? `¥${o.unit_price}` : "-"}</div>
                      </div>
                      <div className="rounded-xl border border-gray-100 bg-white/70 px-3 py-2">
                        <div className="text-[11px] text-gray-500">密码后4位</div>
                        <div className="mt-1 font-mono font-semibold text-gray-900">{o.password_last4}</div>
                      </div>
                      <div className="rounded-xl border border-gray-100 bg-white/70 px-3 py-2">
                        <div className="text-[11px] text-gray-500">创建时间</div>
                        <div className="mt-1 font-semibold text-gray-900">{String(o.created_at).slice(0, 16).replace("T", " ")}</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap justify-end gap-2">
                      ...
                      {o.status === "active" && !o.is_confirmed && (
                        <Button tone="ghost" size="sm" type="button" onClick={() => void doConfirm(o.id)}>
                          确认收货
                        </Button>
                      )}
                      {o.status === "active" && (
                        <>
                          <Button tone="danger" size="sm" type="button" onClick={() => void doRefund(o.id)}>
                            退款吊销
                          </Button>
                          <Button tone="ghost" size="sm" type="button" onClick={() => void doResetPw(o.id)}>
                            重置密码
                          </Button>
                        </>
                      )}
          ...
          <div className="glass w-full max-w-lg rounded-2xl shadow-soft overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex gap-2">
                <Button tone="ghost" size="sm" type="button" onClick={() => copy(resetRes.copy_text)}>
                  复制文案
                </Button>
                <Button tone="ghost" size="sm" type="button" onClick={() => window.open(`/view/${resetRes.order_id}`, "_blank")}>
                  打开阅读页
                </Button>
              </div>
            </div>
